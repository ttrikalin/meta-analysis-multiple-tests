# this is the modelfile for joint meta-analysis 
# for humerus and femur data 
# treating the studies as parallel designs 
#
...
#### Both tests, complete crosstabs for TPR and FPR: group 1
    for(k1 in 1:K.G1) {
        ...
        x.down.11.G1[k1] ~ dbin(p.down.11.G1[k1], N.down.G1[k1])
        x.down.1dot.G1[k1] ~ dbin(p.down.1dot.G1[k1], N.down.G1[k1])
        x.down.dot1.G1[k1] ~ dbin(p.down.dot1.G1[k1], N.down.G1[k1])
       
        # p11.DOWN <-ilogit(eta3)
        p.down.11.G1[k1] <- ilogit(eta.xi.G1[k1,3])
        ...
        # p11.NODOWN
        x.nodown.11.G1[k1] ~ dbin(p.nodown.11.G1[k1], N.nodown.G1[k1])
        x.nodown.1dot.G1[k1] ~ dbin(p.nodown.1dot.G1[k1], N.nodown.G1[k1])
        x.nodown.dot1.G1[k1] ~ dbin(p.nodown.dot1.G1[k1], N.nodown.G1[k1])
       
        # p11.nodown <-ilogit(eta3)
        p.nodown.11.G1[k1] <- ilogit(eta.xi.G1[k1,6])
        # Sensitivity for test 1 (humerus)
    ...
    for(k4 in 1:K.G4) {
        # observational part of the model
        x.down.dot1.G4[k4] ~ dbin(p.down.dot1.G4[k4], N.down.G4[k4])
        x.nodown.dot1.G4[k4] ~ dbin(p.nodown.dot1.G4[k4], N.nodown.G4[k4])
    
        p.down.dot1.G4[k4] <- ilogit(eta.xi.G4[k4, 1]) 
        p.nodown.dot1.G4[k4] <- ilogit(eta.xi.G4[k4, 2]) 

        # structural part of the model 
        eta.xi.G4[k4, 1:2] ~ dmnorm(Eta.Xi.G4[1:2], Omega.G4[1:2, 1:2])
    }
    ...
    # Omega.G4 and Eta.Xi.G4
    # I <- diag(rep(1,6))
    # P.G4 <- I[,c(2, 5, 1, 3, 4, 6)]
    Omega.G4 <- t(P.G4) %*% Omega %*% P.G4 
    Eta.Xi.G4 <- Eta.Xi %*% P.G4 
